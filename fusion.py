...
class Fusion_Module(nn.Module):
    ...
    def forward(self, x_rgb, x_tir, x_sum):
        rgb_fusion = self.drop_path(self.fusin_attn(self.norm1(x_rgb), self.norm2(x_tir)))
        tir_fusion = self.drop_path(self.fusin_attn(self.norm2(x_tir), self.norm1(x_rgb)))
        res = rgb_fusion + tir_fusion + self.norm1(x_sum)
        res = res + self.drop_path(self.mlp(self.norm3(res)))

        res = self.drop_path(self.fusin_attn(res, res)) + res
        res = res + self.drop_path(self.mlp(self.norm3(res)))
